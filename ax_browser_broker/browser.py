...
def _is_closed_transport_error(error: Exception) -> bool:
    text = repr(error)
    return any(marker in text for marker in _CLOSED_TRANSPORT_MARKERS)
...
class BrowserController:
    ...
    async def _reset_lease_connection(self, lease: Lease) -> None:
    ...
    async def _with_transport_recovery(self, lease: Lease, action: Any) -> Any:
        try:
            ...
            if not _is_closed_transport_error(error):
                raise
            await self._reset_lease_connection(lease)
            return await action()
    ...
    async def screenshot(self, lease: Lease, full_page: bool = False) -> dict[str, Any]:
        async def action() -> dict[str, Any]:
            ...
            try:
                ...
                if _is_closed_transport_error(error):
                    raise
